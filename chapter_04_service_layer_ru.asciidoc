...
Это зависит от репозитория.  Мы решили сделать зависимость явной и использовали подсказку типа, чтобы сказать, что мы зависим от `AbstractRepository`. Это означает, что он будет работать и тогда, когда тесты предоставят ему `FakeRepository`, и когда приложение Flask предоставит ему `SqlAlchemyRepository`.
...
[[why_is_everything_a_service]]
=== Почему все называется сервисом?
...
В этой главе мы используем две вещи, называемые _service_. Первый-это _application service_ (наш уровень обслуживания). Его работа заключается в обработке запросов из внешнего мира и в _orchestrate _ операции. Мы имеем в виду, что уровень сервиса управляет приложением, следуя нескольким простым шагам:

* Получить некоторые данные из базы данных
* Обновить модели домена
* Сохранить любые изменения

Это рутина, которая должна выполняться для каждой операции в вашей системе, и отделение ее от бизнес-логики помогает поддерживать порядок.
