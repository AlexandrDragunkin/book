...
== Пример первого использования: [.keep-together]#Flask API и Service Layer#
...
В этой главе мы обсудим различие между логическим построением, бизнес - логикой и кодом интерфейса, а также введем модель  _Service Layer_ для координации наших бизнес - процессов и определения вариантов использования системы.
...
Как и любая другая команда, быстрая и проворная, мы пытаемся получить MVP и собрать обратную связь на глазах у пользователей. У нас есть ядро нашей доменной модели и доменная служба, необходимая для распределения заказов, а также интерфейс репозитория для постоянного хранилища.

Давайте как можно скорее соединим все мобильные компоненты и воссоздадим их в более чистую архитектуру. Наш план таков:


1. Используем Flask, чтобы поместить терминал API перед сервисом "allocate".
    Подключаем сеанс базы данных и наш репозиторий. Тестируем его с помощью сквозного теста и некоторого quick-and-dirty SQL для подготовки тестовых данных.    Проведём сеанс работы с базой данных и нашим репозиторием. Протестируйте его с помощью сквозного теста и небольшого количества quick-and-dirty запросов SQL для подготовки тестовых данных.
   ((("Flask framework", "putting API endpoint in front of allocate domain service")))

2. Рефакторинг уровня сервиса, который может служить абстракцией для захвата варианта использования и который будет находиться между Flask и нашей моделью домена.    Построим несколько тестов уровня сервиса и покажем, как они могут использовать `FakeRepository`.
...
<1> `random_sku()`, `random_batchref ()` и так далее -- это небольшие вспомогательные функции, которые генерируют случайные символы с помощью модуля `uuid`. Поскольку сейчас мы работаем с реальной базой данных, это один из способов предотвратить взаимное влияние различных тестов и прогонов.
...
Все решают эти проблемы по-разному, но вам понадобится какой-то способ развернуть Flask, возможно, в контейнере, и поговорить с базой данных Postgres. Если вы хотите увидеть, как мы это сделали, проверьте
...
Но подождите минутку -- никаких обязательств. На самом деле мы не сохраняем наше распределение в базе данных. Теперь нам нужен второй тест, либо тот, который проверит состояние базы данных после (не очень черный ящик), или, может быть, тот, который проверяет, что мы не можем выделить вторую строку, если первая уже должна была исчерпать пакет:
