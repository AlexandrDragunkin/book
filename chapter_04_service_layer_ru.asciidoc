...
Вернемся к нашему проекту распределения товаров! <<maps_service_layer_before>> показывает точку, которую мы достигли в конце <<chapter_02_repository>>, который включает в себя шаблон репозитория.

[role="width-75"]
[[maps_service_layer_before]]
.До: Управляем приложением, общаясь с репозиторием и моделью домена
